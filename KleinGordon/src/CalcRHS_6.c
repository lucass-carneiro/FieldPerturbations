    ...
    for (k = gz; k < cctk_lsh[2] - gz; k++) {
        for (j = gy; j < cctk_lsh[1] - gy; j++) {
            for (i = gx; i < cctk_lsh[0] - gx; i++) {
                ...
                KTraceL = igxxL * kxxL + igyyL * kyyL + igzzL * kzzL + 2 * igxyL * kxyL +
                ...
                Gamma_xyy = 0.5 * (2 * igxxL * d_y_gxy - igxxL * d_x_gyy + igxyL * d_y_gyy -
                                   igxzL * d_z_gyy + 2 * igxzL * d_y_gyz);
                Gamma_xyz =
                    0.5 * (igxyL * d_z_gyy + igxxL * (d_z_gxy + d_y_gxz - d_x_gyz) + igxzL * d_y_gzz);
                Gamma_xzz = 0.5 * (2 * igxxL * d_z_gxz + 2 * igxyL * d_z_gyz - igxxL * d_x_gzz -
                ...
                Gamma_yyy = 0.5 * (2 * igxyL * d_y_gxy - igxyL * d_x_gyy + igyyL * d_y_gyy -
                                   igyzL * d_z_gyy + 2 * igyzL * d_y_gyz);
                Gamma_yyz =
                    0.5 * (igyyL * d_z_gyy + igxyL * (d_z_gxy + d_y_gxz - d_x_gyz) + igyzL * d_y_gzz);
                Gamma_yzz = 0.5 * (2 * igxyL * d_z_gxz + 2 * igyyL * d_z_gyz - igxyL * d_x_gzz -
                ...
                Gamma_zyy = 0.5 * (2 * igxzL * d_y_gxy - igxzL * d_x_gyy + igyzL * d_y_gyy -
                                   igzzL * d_z_gyy + 2 * igzzL * d_y_gyz);
                Gamma_zyz =
                    0.5 * (igyzL * d_z_gyy + igxzL * (d_z_gxy + d_y_gxz - d_x_gyz) + igzzL * d_y_gzz);
                Gamma_zzz = 0.5 * (2 * igxzL * d_z_gxz + 2 * igyzL * d_z_gyz - igxzL * d_x_gzz -
