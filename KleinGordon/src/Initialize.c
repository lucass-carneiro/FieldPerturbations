...
inline CCTK_REAL f(CCTK_REAL x, CCTK_REAL sigma) { return exp(-0.5 * (x / sigma) * (x / sigma)); }
inline CCTK_REAL fx(CCTK_REAL x, CCTK_REAL sigma) { return (-x * f(x, sigma)) / (sigma * sigma); }

CCTK_REAL exact_gaussian(CCTK_REAL t, CCTK_REAL x, CCTK_REAL y, CCTK_REAL z) {
  DECLARE_CCTK_PARAMETERS;

  const CCTK_REAL r
      = sqrt((x - gaussian_x0) * (x - gaussian_x0) + (y - gaussian_y0) * (y - gaussian_y0)
...
CCTK_REAL dt_exact_gaussian(CCTK_REAL t, CCTK_REAL x, CCTK_REAL y, CCTK_REAL z) {
  DECLARE_CCTK_PARAMETERS;
  ...
  if (r < 1.0e-8) {
    return (2 * (-t + gaussian_sigma) * (t + gaussian_sigma))
           / (exp(t * t / (2. * gaussian_sigma * gaussian_sigma)) * gaussian_sigma * gaussian_sigma
              * gaussian_sigma * gaussian_sigma);
  } else {
    return (r + exp((2 * r * t) / (gaussian_sigma * gaussian_sigma)) * (r - t) + t)
           / (exp(((r + t) * (r + t)) / (2 * gaussian_sigma * gaussian_sigma)) * r * gaussian_sigma
              * gaussian_sigma);
  }
}
...
CCTK_REAL Ylm(const CCTK_REAL *buffer, CCTK_INT l, CCTK_INT m, CCTK_REAL theta) {
  const CCTK_REAL legendre_part = buffer[gsl_sf_legendre_array_index(l, abs(m))];
...
CCTK_REAL multipolar_gaussian(CCTK_REAL *buffer, CCTK_INT lmax, CCTK_REAL x, CCTK_REAL y,
                              CCTK_REAL z) {
  DECLARE_CCTK_PARAMETERS;
  ...
  const CCTK_REAL expo
      = exp(-0.5 * ((R - gaussian_R0) / gaussian_sigma) * ((R - gaussian_R0) / gaussian_sigma));

  /* Compute all spherical harmonics up to lmax at this point */
  const CCTK_INT ierr = gsl_sf_legendre_array_e(GSL_SF_LEGENDRE_SPHARM, lmax, theta, -1, buffer);
  ...
  CCTK_REAL multipole_sum = multipoles[0] * Ylm(buffer, 0, 0, atan2(ymy0, xmx0))
