...
extern "C" void FCKleinGordon_calc_rhs(CCTK_ARGUMENTS) {
  ...
  auto F_x = [&](CCTK_INT i, CCTK_INT j, CCTK_INT k) {
    const CCTK_INT ijk = CCTK_GFINDEX3D(cctkGH, i, j, k);
    using std::sqrt;
    return alp[ijk] * sqrt(det_gamma(i, j, k))
  ...
  sbp_coefficients<derivative_direction::x> coeffs_x(cctkGH, cctk_lsh[0], CCTK_DELTA_SPACE(0));
  sbp_coefficients<derivative_direction::y> coeffs_y(cctkGH, cctk_lsh[1], CCTK_DELTA_SPACE(1));
  sbp_coefficients<derivative_direction::z> coeffs_z(cctkGH, cctk_lsh[2], CCTK_DELTA_SPACE(2));
...
#pragma omp parallel
  CCTK_LOOP3_INT(loop_rhs, cctkGH, i, j, k) {
    ...
    Pi_rhs[ijk]
        = alp[ijk] * sqrt(det_gamma(i, j, k)) * field_mass * field_mass * Phi[ijk]
          - global_d<derivative_direction::x>(coeffs_x, coeffs_y, coeffs_z, F_x, jac, i, j, k)
          - global_d<derivative_direction::y>(coeffs_x, coeffs_y, coeffs_z, F_y, jac, i, j, k)
          - global_d<derivative_direction::z>(coeffs_x, coeffs_y, coeffs_z, F_z, jac, i, j, k);

    Psi_x_rhs[ijk]
        = global_d<derivative_direction::x>(coeffs_x, coeffs_y, coeffs_z, dt_Phi, jac, i, j, k);
    Psi_y_rhs[ijk]
        = global_d<derivative_direction::y>(coeffs_x, coeffs_y, coeffs_z, dt_Phi, jac, i, j, k);
    Psi_z_rhs[ijk]
        = global_d<derivative_direction::z>(coeffs_x, coeffs_y, coeffs_z, dt_Phi, jac, i, j, k);
