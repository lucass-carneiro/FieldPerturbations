...
#pragma omp parallel
  CCTK_LOOP3_INT(loop_flux, cctkGH, i, j, k) {
    ...
    F_Psi[ijk] = alp[ijk] * Pi[ijk] / sqrtg
                 - (betax[ijk] * Psi_x[ijk] + betay[ijk] * Psi_y[ijk] + betaz[ijk] * Psi_z[ijk]);
  }
  CCTK_ENDLOOP3_INT(loop_flux);
}
