...
public:
  ...
  const Arith::vect<CCTK_REAL, 3> grad(const Loop::PointDesc &p) const {
    const Arith::vect<CCTK_REAL, 3> vals = {(gf(-2 * p.DI[0] + p.I) - 8 * gf(-p.DI[0] + p.I)
        ...
        = {(-30 * gf(p.I) - gf(-2 * p.DI[0] + p.I) + 16 * gf(-p.DI[0] + p.I)
           ...
           (-30 * gf(p.I) - gf(-2 * p.DI[2] + p.I) + 16 * gf(-p.DI[2] + p.I)
            + 16 * gf(p.DI[2] + p.I) - gf(2 * p.DI[2] + p.I))
               * (1.0 / (12 * power<2>(p.dz))),
           (gf(-2 * p.DI[0] - 2 * p.DI[1] + p.I) - 8 * gf(-p.DI[0] - 2 * p.DI[1] + p.I)
            + 8 * gf(p.DI[0] - 2 * p.DI[1] + p.I) - gf(2 * p.DI[0] - 2 * p.DI[1] + p.I)
            ...
            - 8 * gf(p.DI[0] + 2 * p.DI[1] + p.I) + gf(2 * p.DI[0] + 2 * p.DI[1] + p.I))
               * (1.0 / (144 * p.dx * p.dy)),
           (gf(-2 * p.DI[0] - 2 * p.DI[2] + p.I) - 8 * gf(-p.DI[0] - 2 * p.DI[2] + p.I)
            + 8 * gf(p.DI[0] - 2 * p.DI[2] + p.I) - gf(2 * p.DI[0] - 2 * p.DI[2] + p.I)
            ...
            - 8 * gf(p.DI[0] + 2 * p.DI[2] + p.I) + gf(2 * p.DI[0] + 2 * p.DI[2] + p.I))
               * (1.0 / (144 * p.dx * p.dz)),
           (gf(-2 * p.DI[1] - 2 * p.DI[2] + p.I) - 8 * gf(-p.DI[1] - 2 * p.DI[2] + p.I)
            + 8 * gf(p.DI[1] - 2 * p.DI[2] + p.I) - gf(2 * p.DI[1] - 2 * p.DI[2] + p.I)
