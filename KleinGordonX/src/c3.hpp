...
public:
  ...
  const Arith::vect<CCTK_REAL, 6> hess(const Loop::PointDesc &p) const {
    const Arith::vect<CCTK_REAL, 6> vals
        = {(-2 * gf(p.I) + gf(-p.DI[0] + p.I) + gf(p.DI[0] + p.I)) * (1.0 / (power<2>(p.dx))),
           (-2 * gf(p.I) + gf(-p.DI[1] + p.I) + gf(p.DI[1] + p.I)) * (1.0 / (power<2>(p.dy))),
           (-2 * gf(p.I) + gf(-p.DI[2] + p.I) + gf(p.DI[2] + p.I)) * (1.0 / (power<2>(p.dz))),
