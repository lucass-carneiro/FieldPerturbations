...
extern "C" void KleinGordonX_RHS_4(CCTK_ARGUMENTS) {
    ...
    auto rhs_lambda = [&](const PointDesc &p) {
        ...
        const CCTK_REAL Gamma_xxx = 0.5 * (igxxL * d_x_gxx - igxyL * d_y_gxx - igxzL * d_z_gxx +
        ...
        const CCTK_REAL Gamma_xyy = 0.5 * (2 * igxxL * d_y_gxy - igxxL * d_x_gyy + igxyL * d_y_gyy -
                                           igxzL * d_z_gyy + 2 * igxzL * d_y_gyz);
        const CCTK_REAL Gamma_xyz =
            0.5 * (igxyL * d_z_gyy + igxxL * (d_z_gxy + d_y_gxz - d_x_gyz) + igxzL * d_y_gzz);
        const CCTK_REAL Gamma_xzz = 0.5 * (2 * igxxL * d_z_gxz + 2 * igxyL * d_z_gyz -
                                           igxxL * d_x_gzz - igxyL * d_y_gzz + igxzL * d_z_gzz);

        const CCTK_REAL Gamma_yxx = 0.5 * (igxyL * d_x_gxx - igyyL * d_y_gxx - igyzL * d_z_gxx +
        ...
        const CCTK_REAL Gamma_yyy = 0.5 * (2 * igxyL * d_y_gxy - igxyL * d_x_gyy + igyyL * d_y_gyy -
                                           igyzL * d_z_gyy + 2 * igyzL * d_y_gyz);
        const CCTK_REAL Gamma_yyz =
            0.5 * (igyyL * d_z_gyy + igxyL * (d_z_gxy + d_y_gxz - d_x_gyz) + igyzL * d_y_gzz);
        const CCTK_REAL Gamma_yzz = 0.5 * (2 * igxyL * d_z_gxz + 2 * igyyL * d_z_gyz -
                                           igxyL * d_x_gzz - igyyL * d_y_gzz + igyzL * d_z_gzz);

        const CCTK_REAL Gamma_zxx = 0.5 * (igxzL * d_x_gxx - igyzL * d_y_gxx - igzzL * d_z_gxx +
        ...
        const CCTK_REAL Gamma_zyy = 0.5 * (2 * igxzL * d_y_gxy - igxzL * d_x_gyy + igyzL * d_y_gyy -
                                           igzzL * d_z_gyy + 2 * igzzL * d_y_gyz);
        const CCTK_REAL Gamma_zyz =
            0.5 * (igyzL * d_z_gyy + igxzL * (d_z_gxy + d_y_gxz - d_x_gyz) + igzzL * d_y_gzz);
        const CCTK_REAL Gamma_zzz = 0.5 * (2 * igxzL * d_z_gxz + 2 * igyzL * d_z_gyz -
        ...
        const CCTK_REAL K_Phi_rhs_p2 =
            ...
            d_x_Phi * (igxxL * Gamma_xxx + 2 * igxyL * Gamma_xxy + 2 * igxzL * Gamma_xxz +
